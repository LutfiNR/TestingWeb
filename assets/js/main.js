...
function renderPage() {
	...
	let currentAngle = 0;
	const getAngle = (e) => {
		const rect = calenderContainer.getBoundingClientRect();
		const cornerX = rect.left;
		const cornerY = rect.top;
		const deltaX = e.clientX - cornerX;
		const deltaY = e.clientY - cornerY;
	...
	calenderContainer.addEventListener('mousedown', (e) => {
		mouseDown = e.clientY;
		if (mouseDown<150) {
			calenderContainer.style.transformOrigin= "top left";
			isRotating = true;
			initialAngle = getAngle(e) - currentAngle;
			if (!sound)
				return;
			...
			calenderContainer.addEventListener('mousemove', (e) => {
				if (isRotating) {
					const angle = getAngle(e);
					currentAngle = angle - initialAngle;
					calenderContainer.style.transform = `rotate(${currentAngle}deg)`;
				}
				
			});
			calenderContainer.addEventListener('mouseup', (e) => {
				let mouseUp = e.clientY;
				if (mouseUp > getAngle(e)) {
	...
	flexColumnGap4.className = "d-flex flex-column gap-2 default-cursor";
	calenderContainer.appendChild(flexColumnGap4);

	let imageDiv = document.createElement("div");
	let tearHere = document.createElement("div")
	tearHere.textContent = "<-- tear here -->"
	tearHere.classList.add("tear-here")
	flexColumnGap4.appendChild(tearHere)
